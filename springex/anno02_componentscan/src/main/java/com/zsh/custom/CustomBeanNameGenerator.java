...
public class CustomBeanNameGenerator implements BeanNameGenerator {
    ...
    @Override
    public String generateBeanName(BeanDefinition definition, BeanDefinitionRegistry definitionRegistry) {
        //定义bean的名称
        String beanName=null;

        //1判断当前bean的定义信息是否时注解的
        if(definition instanceof AnnotatedBeanDefinition){
            //2把definition转成注解的bean定义信息
          ...
          //5 遍历types集合
            for (String type : types) {
                ...
                //7判断attributes是否为null 同时必须是@Componet及其衍生注解
                if (attributes != null) {
