...
public class ExceptionDemo {
    public static void main(String[] args) {
        try {
            checkAge(33);
        } catch (ahutAgeIlleagalException e) {
            e.printStackTrace();
        }
        checkAge2(-34);
    }
    public static void checkAge(int age) throws ahutAgeIlleagalException {
        if(age<0||age>200){
            //抛出去一个异常对象给调用者
            //throw 在方法内部直接创建一个异常对象 并从点抛出
            //throws 用在方法上面 抛出方法内部的异常
            throw new ahutAgeIlleagalException(age+"is illeagal!");
        }else{
    ...
    public static void checkAge2(int age)  {
        if(age<0||age>200){
            ...
            //throw 在方法内部直接创建一个异常对象 并从点抛出
            //throws 用在方法上面 抛出方法内部的异常
            throw new ahutAgeIlleagalRuntimeException(age+"is illeagal!");
        }else{
            System.out.println("年龄合法:推荐商品给其购买");
        }
    }
